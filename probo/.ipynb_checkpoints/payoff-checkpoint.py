...
# The same equation used to calculate the Option. A(bar) + (G - G(bar))
# def arthimetic_payoff(prc, price, prices)
    # return prc + (price - prices)
...
def geometricAsianPutPayoff(option, spot):
    average = spot.mean()
    return maximum(option.strike - average, 0.0)
